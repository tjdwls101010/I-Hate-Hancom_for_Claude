...
- [I. 한글 5.0 파일 구조](#i-한글-50-파일-구조)
  - [1. 개요](#1-개요)
  - [2. 자료형 설명](#2-자료형-설명)
  - [3. 한글 파일 구조](#3-한글-파일-구조)
    - [3.1. 한글 파일 구조 요약](#31-한글-파일-구조-요약)
    - [3.2. 스토리지 별 저장 정보](#32-스토리지-별-저장-정보)
  - [4. 데이터 레코드](#4-데이터-레코드)
    - [4.1. 데이터 레코드 구조](#41-데이터-레코드-구조)
    - [4.2. '문서 정보'의 데이터 레코드](#42-문서-정보의-데이터-레코드)
    - [4.3. '본문'의 데이터 레코드](#43-본문의-데이터-레코드)
    - [4.4. 문서 이력 관리](#44-문서-이력-관리)
- [변경 사항 이력](#변경-사항-이력)
...
| DWORD | 4 | 파일 버전. 0xMMnnPPrr의 형태(예 5.0.3.0) § MM: 문서 형식의 구조가 완전히 바뀌는 것을 나타냄. 숫자가 다르면 구 버전과 호환 불가능. § nn: 큰 구조는 동일하나, 큰 변화가 있는 것을 나타냄. 숫자가 다르면 구 버전과 호환 불가능. § PP: 구조는 동일, Record가 추가되었거나, 하위 버전에서 호환되지 않는 정보가 추가된 것을 나타냄. 숫자가 달라도 구 버전과 호환 가능. § rr: Record에 정보들이 추가된 것을 나타냄. 숫자가 달라도 구 버전과 호환 가능. |
| DWORD | 4 | 속성 (아래 참조) |
| DWORD | 4 | 속성 (CCL/공공누리 관련, 아래 참조) |
| DWORD | 4 | EncryptVersion: 0=None, 1=한글 2.5 이하, 2=한글 3.0 Enhanced, 3=한글 3.0 Old, 4=한글 7.0 이후 |
| BYTE | 1 | 공공누리(KOGL) 라이선스 지원 국가: 6=KOR, 15=US |
| BYTE array[207] | 207 | 예약 |
| **전체 길이** | **256** | |

**파일 인식 정보 - 속성 (첫 번째 DWORD)**
...
본문에 사용 중인 글꼴, 글자 속성, 문단 속성, 탭, 스타일 등에 문서 내 공통으로 사용되는 세부 정보를 담고 있다. DocInfo 스트림에 저장되는 데이터는 다음과 같다.

각각의 세부 정보는 ['문서 정보'의 데이터 레코드](#42-문서-정보의-데이터-레코드)란에서 추가로 다룬다.
...
- **DrmLicense** 스트림에는 DRM Packaging의 Version 정의가 저장된다.
- **DrmRootSect** 스트림에는 암호화 알고리즘이 저장된다.
- **CertDrmHeader** 스트림에는 DRM Packaging의 Version 정의가 저장된다.
...
| --- | --- |
| 자료형 | 설명 |
| DATA | 레코드 Type에 따른 다양한 정보 (스트링, Bit flag등) |
...
- **Size** : 데이터 영역의 길이를 바이트 단위로 나타낸다. 12개의 비트가 모두 1일 때는 데이터 영역의 길이가 4095 바이트 이상인 경우로, 이때는 레코드 헤더에 연이어 길이를 나타내는 DWORD가 추가된다.

**확장 데이터 레코드 구조 (4095 바이트 이상):**
...
| bit 0 | 3D 효과의 유무 |
| bit 1 | 그림자 효과의 유무 |
| bit 2～4 | Slash 대각선 모양: 000=none, 010=slash, 011=LeftTop→Bottom Edge, 110=LeftTop→Right Edge, 111=LeftTop→Bottom & Right Edge |
| bit 5～7 | BackSlash 대각선 모양: 000=none, 010=back slash, 011=RightTop→Bottom Edge, 110=RightTop→Left Edge, 111=RightTop→Bottom & Left Edge |
...
| INT16 | 2 | 그러데이션의 색 수(num). 워디안/한글2002/SE에서는 항상 2 |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| BYTE stream | 8 | 문단 머리 정보(표 39 참조) × 7회 반복 (수준 1～7) |
| WORD | 2 | 번호 형식 길이 (len) |
| WCHAR array[len] | 2×len | 번호 형식. ^n: 레벨 경로 표시 (예: 1.1.1.1.1.1.1), ^N: 마지막에 마침표 추가 |
| UINT16 | 2 | 시작 번호 |
| UINT | 4 | 수준별 시작번호 (5.0.2.5 이상) × 7회 반복 (수준 1～7) |
| WORD | 2 | 확장 번호 형식 길이 (len) × 3회 반복 (수준 8～10) |
...
| INT16 | 2 | 언어 아이디 |
| UINT16 | 2 | 문단 모양 아이디 참조값 (스타일의 종류가 문단인 경우 반드시 지정) |
| UINT16 | 2 | 글자 모양 아이디 (스타일의 종류가 글자인 경우 반드시 지정) |
...
| 1 | PIT_BSTR | WORD + WCHAR array | 문자열 길이(slen) + 문자열 |
| 2 | PIT_I1 | UINT | INT8 |
| 3 | PIT_I2 | UINT | INT16 |
| 4 | PIT_I4 | UINT | INT32 |
| 5 | PIT_I | UINT | INT |
...
| 0x8001 | PIT_ARRAY | INT16 + ParameterArray | 파라미터 셋 개수 + 파라미터 셋 배열 |
...
| UINT32 | 4 | control mask. `(UINT32)(1<<ctrlch)` 조합 |
...
| INT32 | 4 | 컬럼에서의 시작 위치 |
| INT32 | 4 | 세그먼트의 폭 |
| UINT32 | 4 | 태그: bit 0=페이지의 첫 줄 여부, bit 1=컬럼의 첫 줄 여부, bit 16=빈 세그먼트 여부, bit 17=줄의 첫 세그먼트 여부, bit 18=줄의 마지막 세그먼트 여부, bit 19=auto-hyphenation 수행 여부, bit 20=indentation 적용, bit 21=문단 머리 모양 적용, bit 31=구현상의 편의를 위한 property |
| **전체 길이** | **36** | |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| UINT32 | 4 | 컨트롤 ID. 컨트롤 ID 이하 속성들은 CtrlID에 따라 다르다. |
| **전체 길이** | **4** | |
...
| --- | --- | --- |
| INT16 | 2 | 문단 수 |
| UINT32 | 4 | 속성: bit 0～2=텍스트 방향(0:가로, 1:세로), bit 3～4=문단의 줄바꿈(0:일반, 1:자간 조종하여 한 줄 유지, 2:내용에 따라 폭이 늘어남), bit 5～6=세로 정렬(0:top, 1:center, 2:bottom) |
| **전체 길이** | **6** | |
...
extended type의 컨트롤은 종류를 나타내는 식별 기호로 32 비트 ID가 사용된다.

`MAKE_4CHID(a, b, c, d)` = `(((a) << 24) | ((b) << 16) | ((c) << 8) | (d))`

**표 67 개체 공통 속성을 포함하는 컨트롤과 컨트롤ID**

| | 컨트롤 | 컨트롤 ID | 개체 공통 속성 | 개체 요소 속성 |
| --- | --- | --- | --- | --- |
...
| | | 1 | page |
| | | 2 | para |
| bit 5～7 | 세로 위치 상대적 배열 방식 | 0 | top (paper/page일 때) / left (그 외) |
| | | 1 | center |
| | | 2 | bottom (paper/page일 때) / right (그 외) |
| | | 3 | inside |
| | | 4 | outside |
...
| | | 2 | column |
| | | 3 | para |
| bit 10～12 | HorzRelTo에 대한 상대적 배열 방식 | | bit 5～7 참조 |
| bit 13 | VertRelTo이 'para'일 때 세로 위치를 본문 영역으로 제한할지 여부 | | |
| bit 14 | 다른 오브젝트와 겹침 허용 여부 | | |
...
| bit 21～23 | 오브젝트 주위 텍스트 흐름 | 0 | Square (bound rect를 따라) |
| | | 1 | Tight (outline을 따라) |
| | | 2 | Through (내부 빈 공간까지) |
| | | 3 | TopAndBottom (좌우에 텍스트 배치하지 않음) |
| | | 4 | BehindText (글 뒤로) |
| | | 5 | InFrontOfText (글 앞으로) |
| bit 24～25 | 좌/우 글 배치 | 0 | BothSides |
| | | 1 | LeftOnly |
| | | 2 | RightOnly |
| | | 3 | LargestOnly |
| bit 26～28 | 번호 범주 | 0 | none |
| | | 1 | figure |
| | | 2 | table |
| | | 3 | equation |
...
| | | 2 | top |
| | | 3 | bottom |
| bit 2 | 캡션 폭에 마진 포함 여부 | | 가로 방향일 때만 사용 |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| WORD | 2 | scale matrix와 rotation matrix쌍의 개수(cnt). 초기엔 1, group할 때마다 증가, ungroup할 때 감소 |
...
| --- | --- | --- |
| bit 0～5 | 선 종류 | |
| bit 6～9 | 선 끝 모양 | 0=round, 1=flat (그림은 "Round", 그리기 개체들은 "Flat"이 디폴트) |
| bit 10～15 | 화살표 시작 모양 | 0=없음, 1=화살, 2=오목한 화살, 3=속 빈 다이아몬드, 4=속 빈 원, 5=속 빈 사각, 6=속 찬 다이아몬드, 7=속 찬 원, 8=속 찬 사각 |
| bit 16～21 | 화살표 끝 모양 | 위와 동일 |
| bit 22～25 | 화살표 시작 크기 | 0=작은-작은, 1=작은-중간, 2=작은-큰, 3=중간-작은, 4=중간-중간, 5=중간-큰, 6=큰-작은, 7=큰-중간, 8=큰-큰 |
| bit 26～29 | 화살표 끝 크기 | 위와 동일 |
...
| INT32 | 4 | 끝점 X 좌표 |
| INT32 | 4 | 끝점 Y 좌표 |
| UINT16 | 2 | 속성. 방향을 바로 잡아주기 위한 플래그. |
| **전체 길이** | **18** | |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| BYTE | 1 | 사각형 모서리 곡률(%). 직각=0, 둥근 모양=20, 반원=50 |
...
| INT32 | 4 | end pos x |
| INT32 | 4 | end pos y |
| INT32 | 4 | start pos x2 (interval of curve, effective only when arc) |
...
| 범위 | 설명 |
| --- | --- |
| bit 0 | 호(ARC)로 바뀌었을 때 interval을 다시 계산해야 할 필요가 있는지 여부 |
...
| INT32 array[4] | 4×4 | 이미지 테두리 사각형의 x 좌표(최초 그림 삽입 시 크기) |
| INT32 array[4] | 4×4 | 이미지 테두리 사각형의 y 좌표(최초 그림 삽입 시 크기) |
...
**표 109 그림자 효과 속성** (전체 길이: 가변, 44 + n 바이트)
...
**표 110 네온 효과 속성** (전체 길이: 가변, 8 + n 바이트)
...
**표 111 부드러운 가장자리 효과 속성** (전체 길이: 4)

| 자료형 | 길이 | 설명 |
| --- | --- | --- |
| float | 4 | 부드러운 가장자리 반경 |

**표 112 반사 효과 속성** (전체 길이: 56)
...
| 범위 | 구분 | 값 | 설명 |
| --- | --- | --- | --- |
| bit 0～7 | | | DVASPECT_CONTENT=1, DVASPECT_THUMBNAIL=2, DVASPECT_ICON=4, DVASPECT_DOCPRINT=8 |
| bit 8 | | | TRUE if moniker is assigned |
| bit 9～15 | 베이스라인 | | 0은 디폴트(85%), 1～101이 0～100% |
...
차트는 본문에 OLE 개체로 저장되며, 차트 내용은 OLE Compound 파일로 저장된다. 차트 파일의 최상위 storage에 "Contents", "OOXMLChartContents" stream이 저장될 수 있다.

기존 hwp(한글 Neo) 차트에서는 "Contents" stream만 존재한다.

한글 2018에서는 OOXML 차트를 저장할 수 있고, "OOXMLChartContents"와 "Contents" stream에 차트를 저장할 수 있다. "Contents" stream은 OOXML 차트를 구 버전(한글 Neo)의 차트로 변환한 예비 데이터이다.
...
| BYTE stream | n3 | 개체 속성 × 묶음 개체의 갯수 (묶음 가능 개체: 그리기 개체, OLE, 그림, 묶음 개체) |
| **전체 길이** | **가변** | n + n2 + n3 바이트 |
...
| HWPUNIT16 | 2 | 세로로 줄맞춤 여부 (0=off, 1-n=간격을 HWPUNIT 단위로 지정) |
| HWPUNIT16 | 2 | 가로로 줄맞춤 여부 (0=off, 1-n=간격을 HWPUNIT 단위로 지정) |
| HWPUNIT | 4 | 기본 탭 간격 |
| UINT16 | 2 | 번호 문단 모양 ID |
| UINT16 | 2 | 쪽 번호 (0=앞 구역에 이어, n=임의의 번호로 시작) |
| UINT16 array[3] | 2×3 | 그림, 표, 수식 번호 (0=앞 구역에 이어, n=임의의 번호로 시작) |
| UINT16 | 2 | 대표Language (5.0.1.5 이상) |
| **전체 길이** | **26** | |
| | | **하위 레코드** |
...
| BYTE stream | 10 | 바탕쪽 정보(표 137 참조). 양 쪽, 홀수 쪽, 짝수 쪽 |
| **하위 전체** | **140** | |
...
| | | 0x80 | 4가지 문자가 차례로 반복 |
| | | 0x81 | 사용자 지정 문자 반복 |
| bit 8～9 | 각주 배치 방법 | 0 | (각주) 각 단마다 따로 배열 / (미주) 문서의 마지막 |
| | | 1 | (각주) 통단으로 배열 / (미주) 구역의 마지막 |
| | | 2 | (각주) 가장 오른쪽 단에 배열 |
| bit 10～11 | numbering | 0 | 앞 구역에 이어서 |
| | | 1 | 현재 구역부터 새로 시작 |
| | | 2 | 쪽마다 새로 시작(각주 전용) |
| bit 12 | 각주 내용 중 번호 코드를 위 첨자 형식으로 할지 여부 | | |
...
| bit 12 | superscript | | 각주에서만 사용. 위 첨자 형식 여부 |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| UINT | 2 | 속성: 0x00000001=머리말, 0x00000002=꼬리말, 0x00000004=바탕쪽, 0x00000008=테두리, 0x00000010=배경, 0x00000020=쪽 번호 위치 |
| **전체 길이** | **2** | |
...
| 자료형 | 길이(바이트) | 설명 |
| --- | --- | --- |
| UINT32 | 4 | 속성: bit 0～1 홀/짝수 구분 (0=양 쪽, 1=짝수 쪽, 2=홀수 쪽) |
| **전체 길이** | **4** | |
...
책갈피 컨트롤은 책갈피로서 갖는 정보로서 '책갈피이름'밖에 없으며, 컨트롤 임의의 데이터인 HWPTAG_CTRL_DATA로 레코드된다.
...
| UINT array[cnt] | 4×cnt | 테두리 내부 글자의 charshapeid의 배열 |
...
| UINT | 4 | Option |
| UINT | 4 | Style number |
| UINT | 4 | 정렬 기준: 0=양쪽, 1=왼쪽, 2=오른쪽, 3=가운데, 4=배분, 5=나눔(공백에만 배분) |
| **전체 길이** | **28** | |
...
한글 메뉴의 "파일-문서 이력 관리"에서 표시 및 생성되는 문서의 이력 정보를 저장하는 장소이다. (한글 2005에서 한글 2007까지는 '버전 비교(파일-버전 비교)'라는 이름으로 '문서 이력 관리 정보' 기능을 제공하였다.)
